...
Runs YOLO inference using CUDA provider (onnxruntime-gpu).
Requires CUDA and onnxruntime-gpu.
"""
...
@workflow_node("cuda_inference",
               dependencies=["onnxruntime-gpu", "numpy", "opencv-python", "Pillow"],
               isolation_mode="subprocess")
def cuda_inference_node(model_session: str = None, 
                        model_info: Dict = None, 
                        confidence_threshold: float = 0.25,
                        iterations: int = 10,
                        image_data: List = None):
    """Run inference using CUDA (onnxruntime-gpu)"""
    try:
        ...
        # Warmup run (CUDA needs this for kernel compilation)
        ...
        print(f"CUDA: Detected {len(formatted_detections)} objects")
        ...
        return {"error": f"CUDA inference failed: {str(e)}"}
