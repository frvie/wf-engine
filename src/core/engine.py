...
from src.utils.logging_config import get_workflow_logger
...
class FunctionWorkflowEngine:
    ...
    def _initialize_environment_manager(self):
        ...
        try:
            from src.core.environment_manager import WorkflowEnvironmentManager
    ...
    def _discover_and_load_nodes(self):
        ...
        # Scan recursively for all Python files in src/nodes
        for py_file in nodes_dir.rglob("*.py"):
            if py_file.name == '__init__.py':
                continue
                
            try:
                # Convert path to module name (e.g., src/nodes/image_ops.py -> src.nodes.image_ops)
...
try:
    from src.utils.shared_memory_utils import (
